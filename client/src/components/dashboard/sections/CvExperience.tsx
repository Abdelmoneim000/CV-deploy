...
const CvExperience = ({
  ...
  const section = cvData?.data?.sections.find((s) => s.id === 'experience');
  const TITLE_PLACEHOLDER = 'Experience';
  ...
  const handleEntryChange = (
    index: number,
    key: 'company' | 'position' | 'startDate' | 'endDate' | 'description',
    value: string
  ) => {
    if (!editable) return; // Prevent changes if not editable
      ...
      {editable && showToolkit && (
        ...
        >
          <div className="font-semibold text-gray-700 flex items-center gap-2">
            <FiSettings /> Experience Settings
          </div>

          {/* Visibility Controls */}
          <div className="space-y-2">
            {/* Field Visibility Controls */}
            {['company', 'position', 'dates', 'description'].map((field) => (
          ...
          <li key={index} className="group">
            {isVisible('company') && (
              <div className="flex justify-between gap-2">
                <EditableDiv
                  value={typeof entry.company === 'string' ? entry.company : ''}
                  placeholder="Company"
                  className="text-md font-semibold w-full outline-none bg-transparent"
                  onChange={(val) => handleEntryChange(index, 'company', val)}
                  editable={editable}
                />
            ...
            {isVisible('position') && (
              <EditableDiv
                value={typeof entry.position === 'string' ? entry.position : ''}
                placeholder="Position"
                className="text-sm text-gray-700 bg-transparent outline-none"
                onChange={(val) => handleEntryChange(index, 'position', val)}
                editable={editable}
              />
            )}
